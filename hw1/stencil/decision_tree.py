...
class DecisionTree:
    ...
    def _prune_recurs(self, node, validation_data):
        ...
        if not node is None:
            ...
            data_left = [row for row in validation_data if row[node.index_split_on] == True]
            data_right = [row for row in validation_data if row[node.index_split_on] == False]
            if not node.isleaf:
    ...
    def _split_recurs(self, node, rows, indices):
        ...
        data_left = [row for row in rows if row[node.index_split_on] == True]
        data_right = [row for row in rows if row[node.index_split_on] == False]
        if len(data_left) != 0:
            node.left = Node(depth=node.depth + 1, label=1)
        if len(data_right) != 0:
            node.right = Node(depth=node.depth + 1, label=0)
        label_0_cnt, label_1_cnt = 0, 0
        for row in rows:
            if self.predict(row) == 1:
                label_1_cnt += 1
            else:
                label_0_cnt += 1
        node.label = 1 if label_1_cnt > label_0_cnt else 0
